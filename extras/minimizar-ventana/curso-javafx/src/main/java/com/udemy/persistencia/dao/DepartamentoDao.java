...
public class DepartamentoDao {
    ...
    public Map<String, Integer> contarEmpleadosPorDepartamento() {
        ...
        try (Connection conexionDB = DriverManager.getConnection(URL_CONEXION, USUARIO_BDD, PASSWORD_BDD)) {
            Statement statement = conexionDB.createStatement();
            String sql = "SELECT id_departamento, count(*) as cantidad FROM empleado GROUP BY id_departamento";
            ResultSet resultSet = statement.executeQuery(sql);
            while (resultSet.next()) {
                int idDepartamento = resultSet.getInt("id_departamento");
                int cantidadEmpleados = resultSet.getInt("cantidad");

                // busca el departamento en la lista
                for (Departamento departamento : departamentos) {
                    if (departamento.getId() == idDepartamento) {
                        empleadosPorDepartamento.put(departamento.getNombre(), cantidadEmpleados);
                        break;
                    }
                }
            }
